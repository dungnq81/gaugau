...
if (! class_exists('ACF_Post_Type')) {
    class ACF_Post_Type extends ACF_Internal_Post_Type
    {
        ...
        public function validate_post($post = [])
        {
            ...
            foreach ($post as $setting => $value) {
                if (isset($defaults[ $setting ])) {
                    ...
                    if ('boolean' === $default_type && in_array($value, ['0', '1'], true)) {
                        $post[ $setting ] = (bool) $value;
                    }

                    if ('boolean' === $default_type && in_array($value, ['false', 'true'], true)) {
        ...
        public function build_safe_context_for_metabox_cb($post)
        {
            ...
            $unset     = ['_POST', '_GET', '_REQUEST', '_COOKIE', '_SESSION', '_FILES', '_ENV', '_SERVER'];
            $originals = [];

            foreach ($unset as $var) {
                if (isset($GLOBALS[ $var ])) {
                    $originals[ $var ] = $GLOBALS[ $var ];
                    $GLOBALS[ $var ]   = []; //phpcs:ignore -- used for building a safe context
                }
            }
